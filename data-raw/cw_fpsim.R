...
# Okie doke, let's define an S function for stuff to follow:
...
SALLYVS <- foreach(
  ...
  smatrix <- matrix(NA, nrow = n, ncol = n, dimnames = list(ccodes, ccodes))
  A$ccode1 <- NULL

  for (i in 1:n) {
    for (j in i:n) {
      sscores <- srs(A[i, ], A[j, ], data = "alliances", distances = "squared", ordered = FALSE)
      smatrix[i, j] <- sscores
      smatrix[j, i] <- sscores  # symmetric
    }
  }
...
SALLYVA <- foreach(
  ...
  for (i in 1:n) {
    for (j in i:n) {
      sscores <- srs(A[i, ], A[j, ], data = "alliances", distances = "absolute", ordered = FALSE)
      smatrix[i, j] <- sscores
      smatrix[j, i] <- sscores  # symmetric
    }
  }
...
# This is squared distances
srs <- function(data, distances = "squared") {
  if(distances == "squared") {
    ...
    numer <- sum(abs(data$cub_ordatop - data$can_ordatop))
    denom <- length(data$can_ordatop)*max(data$can_ordatop)
    x <- 1 - 2*(numer/denom)
...
for (i in 1:n) {
  for (j in i:n) {
    sim <- srs(A[i, ], A[j, ], type = 'absolute')
    s_matrix2[i, j] <- sim
    s_matrix2[j, i] <- sim  # symmetric
  }
}
identical(s_matrix, s_matrix2)
...
asrs <- function(data) {
  similarity <- sum((0.001670064/1.046032)*(data$cub_ordatop - data$can_ordatop)^2)
